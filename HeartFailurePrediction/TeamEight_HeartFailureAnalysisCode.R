...
#Confusion matrix
addmargins(table(y_pred_test, y_act_test))
confusionMatrix(table(y_pred_test, y_act_test))
...
precision_test = diag1 / rowsums1 
recall_test = diag1 / colsums1 
f1_test = 2 * precision_test * recall_test / (precision_test + recall_test)

data.frame(precision_test, recall_test, f1_test) 

macroPrecision_test = mean(precision_test)
macroRecall_test = mean(recall_test)
macroF1_test = mean(f1_test)
data.frame(macroPrecision_test, macroRecall_test, macroF1_test)

#Finding ROC curve
#library(randomForest)
library(pROC)

predict.score = as.numeric(predict(heart_C5.0_model, testData, type = "class"))

head(predict.score)
roc.multi <- multiclass.roc(testData$DEATH_EVENT, predict.score, percent=TRUE)
auc(roc.multi)
...
precision_train = diag_train / rowsums_train 
recall_train = diag_train / colsums_train
f1_train = 2 * precision_train * recall_train / (precision_train + recall_train)

data.frame(precision_train, recall_train, f1_train) 

macroPrecision_train = mean(precision_train)
macroRecall_train = mean(recall_train)
macroF1_train = mean(f1_train)
data.frame(macroPrecision_train, macroRecall_train, macroF1_train)
